...
TEST(TensorOps, AddBackward) {
  ...
  ps.zero_grad();
  ps.backward(s);
  for (size_t i=0;i<a.numel;++i) { EXPECT_FLOAT_EQ(a.grad()[i], 1.0f); }
  for (size_t i=0;i<b.numel;++i) { EXPECT_FLOAT_EQ(b.grad()[i], 1.0f); }
}

TEST(TensorOps, MulBackward) {
  ParameterStore ps; ps.clear_tape();
  ...
  auto c = mul(a,b,ps);
  auto s = sum(c,ps);
  ps.zero_grad(); ps.backward(s);
  for (size_t i=0;i<a.numel;++i) EXPECT_FLOAT_EQ(a.grad()[i], b.data()[i]);
  for (size_t i=0;i<b.numel;++i) EXPECT_FLOAT_EQ(b.grad()[i], a.data()[i]);
}
