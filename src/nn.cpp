...
Linear::Linear(int in_f, int out_f, ParameterStore &store, bool bias, float init_scale, unsigned seed)
    : in_features(in_f), out_features(out_f), use_bias(bias), W(store.parameter({in_f, out_f}, init_scale, seed)),
      b(bias ? store.parameter({out_f}, init_scale, seed ^ 0xA5A5A5) : Tensor{})
{
}
...
Tensor bce_with_logits_loss(const Tensor &logits, const Tensor &targets, ParameterStore &store, float eps)
{
