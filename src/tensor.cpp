...
void Tensor::zero_grad()
{
    if (!store)
        return;
    std::fill(store->grad_buf.begin() + offset, store->grad_buf.begin() + offset + numel, 0.0f);
}
...
void Tensor::fill(float v)
{
    if (!store)
        return;
    std::fill(store->data_buf.begin() + offset, store->data_buf.begin() + offset + numel, v);
}
...
Tensor ParameterStore::parameter(const std::vector<int> &shape, float scale, unsigned seed)
{
    auto t = tensor(shape);
...
void ParameterStore::clear_tape()
{
    tape.clear();
}
...
static void assert_same_shape(const Tensor &a, const Tensor &b)
{
    ...
    if (a.numel != b.numel)
        throw std::invalid_argument("Numel mismatch");
...
Tensor add_rowwise(const Tensor &X, const Tensor &b, ParameterStore &store)
{
    ...
    int H = X.shape[1];
    if (b.shape[0] != H)
        throw std::invalid_argument("add_rowwise dim mismatch");
