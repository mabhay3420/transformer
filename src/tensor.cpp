...
// Tensor methods
float *Tensor::data() { return store ? store->data_buf.data() + offset : nullptr; }
float *Tensor::grad() { return store ? store->grad_buf.data() + offset : nullptr; }
...
void ParameterStore::zero_grad() { std::fill(grad_buf.begin(), grad_buf.end(), 0.0f); }

void ParameterStore::clear_tape() { tape.clear(); }

void ParameterStore::backward(const Tensor &loss) {
  if (loss.store != this) throw std::invalid_argument("loss belongs to different store");
  // Seed dL/dL = 1
